...
void calc_dist_end() {
	 ...
	 * */
	vehicle.dist = (r_motor.dist + l_motor.dist) / 2;
	if (vehicle.dist >= vehicle.tar_dist) {
		vehicle.end_flag = 1;
	}
}
...
void calc_ang_end() {
	 ...
	 * */
	vehicle.ang = (r_motor.dist - l_motor.dist) / spec.tread;
	if (vehicle.ang >= vehicle.tar_ang) {
		vehicle.end_flag = 2;
	}
}
